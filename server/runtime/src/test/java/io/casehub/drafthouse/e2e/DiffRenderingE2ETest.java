...
class DiffRenderingE2ETest {
    ...
    void deletedBlockOnAside() {
        loadFilePair(page, index, fixturePath("diff-a.md"), fixturePath("diff-b.md"));
        int count = shadowLocator(page, "drafthouse-diff", "#render-a .diff-del").count();
    ...
    void insertedBlockOnBside() {
        loadFilePair(page, index, fixturePath("diff-a.md"), fixturePath("diff-b.md"));
        int count = shadowLocator(page, "drafthouse-diff", "#render-b .diff-ins").count();
    ...
    void minimapCanvasHasNonZeroDimensions() {
        loadFilePair(page, index, fixturePath("diff-a.md"), fixturePath("diff-b.md"));
        assertThat(shadowLocator(page, "drafthouse-diff", "#diff-map")).isVisible();
        int width  = (int) shadowLocator(page, "drafthouse-diff", "#diff-map").evaluate("el => el.width");
        int height = (int) shadowLocator(page, "drafthouse-diff", "#diff-map").evaluate("el => el.height");
